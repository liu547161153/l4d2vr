...
std::string VR::WeaponIdToString(C_WeaponCSBase::WeaponID weaponId) const
{
    ...
    if (rawKey.rfind(weaponPrefix, 0) == 0)
    {
            ...
            std::all_of(weaponIdString.begin(), weaponIdString.end(), [](unsigned char c) { return std::isdigit(c); }))
        {
            try
            {
                ...
                std::string weaponName = WeaponIdToString(static_cast<C_WeaponCSBase::WeaponID>(weaponId));
                if (!weaponName.empty())
