// Guard against occasional stale client entity pointers during DrawModelExecute.
// In those cases, directly dereferencing netvars can AV. Use SEH to fail closed.
#ifdef _MSC_VER
#include <windows.h>
#endif

static bool SafeReadInt32(const void* addr, int& out)
{
#ifdef _MSC_VER
    __try
    {
        out = *reinterpret_cast<const volatile int*>(addr);
        return true;
    }
...
#else
    // Non-MSVC builds (unlikely for this project): best-effort read.
...
VR::SpecialInfectedType VR::GetSpecialInfectedType(const C_BaseEntity* entity) const
{
    ...
    int zombieClass = 0;
    if (!SafeReadInt32(base + kZombieClassOffset, zombieClass))
        return SpecialInfectedType::None;
...
{
    // Special infected auto-aim now uses only the local/controller-side aim path.
    // The synthetic secondary RunCommand prediction path is disabled because it is
    // currently unstable and can desync the auto-aim solution from what the player
    // is actually seeing locally.
    (void)cmd;
    return false;
}
