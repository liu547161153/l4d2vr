...
Option g_Options[] =
{
    ...
    // Mock VR (no SteamVR/OpenVR)
    {
        "UseMockVR",
        OptionType::Bool,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Use Mock OpenVR Backend", u8"启用模拟OpenVR后端" },
        { u8"Runs VR logic without SteamVR/OpenVR by generating synthetic HMD + controller poses.",
          u8"不依赖SteamVR/OpenVR，直接生成HMD与左右手的模拟姿态来运行VR逻辑。" },
        { u8"Startup-only: restart required after changing this option.",
          u8"仅启动时生效：修改后需要重启游戏。" },
        0.0f, 0.0f,
        "false"
    },
    {
        ...
        OptionType::Bool,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Enable Keyboard/Mouse Control", u8"启用键鼠控制" },
        { u8"Drives mock poses using keyboard/mouse input.",
          u8"使用键盘/鼠标输入来驱动模拟姿态。" },
        { u8"WASD move, mouse look (optional), PageUp/PageDown up/down.",
          u8"WASD移动，鼠标视角（可选），PageUp/PageDown上下移动。" },
        0.0f, 0.0f,
        "true"
    },
    {
        ...
        OptionType::Bool,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Mouse Look", u8"鼠标控制视角" },
        { u8"Allows mouse movement to rotate the mock HMD (yaw/pitch).",
          u8"允许用鼠标移动来旋转模拟HMD（偏航/俯仰）。" },
        { u8"Disable if you want scripted/static angles.",
          u8"如果你希望脚本/固定角度驱动，可以关闭。" },
        0.0f, 0.0f,
        "true"
    },
    {
        "MockVRMouseLookRequiresRMB",
        OptionType::Bool,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Mouse Look Requires RMB", u8"鼠标视角需按住右键" },
        { u8"Only rotate the mock HMD while holding right mouse button.",
          u8"只有按住鼠标右键时才旋转模拟HMD。" },
        { u8"Useful when you still need the mouse cursor for UI.",
          u8"当你仍需要鼠标指针操作UI时很有用。" },
        0.0f, 0.0f,
        "false"
    },
    {
        "MockVRMouseSensitivity",
        OptionType::Float,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Mouse Sensitivity (deg/pixel)", u8"鼠标灵敏度（度/像素）" },
        { u8"Rotation sensitivity for mouse look in mock mode.",
          u8"模拟模式下鼠标视角的旋转灵敏度。" },
        { u8"Higher = faster turning.",
          u8"越大转得越快。" },
        0.001f, 5.0f,
        "0.12"
    },
    {
        ...
        OptionType::Float,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Move Speed (m/s)", u8"移动速度（米/秒）" },
        { u8"Translation speed for mock HMD movement.",
          u8"模拟HMD平移的移动速度。" },
        { u8"Used by WASD / vertical keys when Keyboard/Mouse control is enabled.",
          u8"启用键鼠控制时，由WASD/上下按键使用。" },
        0.0f, 50.0f,
        "1.5"
    },
    {
        ...
        OptionType::Float,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Run Multiplier", u8"加速倍率" },
        { u8"Speed multiplier while holding Shift (mock mode).",
          u8"按住Shift时的速度倍率（模拟模式）。" },
        { u8"1.0 means no boost.",
          u8"1.0表示不加速。" },
        1.0f, 50.0f,
        "3.0"
    },
    {
        "MockVRHandsFollowHmd",
        OptionType::Bool,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Hands Follow HMD", u8"双手跟随HMD" },
        { u8"Controllers are derived from HMD pose using offsets (relative mode).",
          u8"使用相对偏移从HMD姿态派生左右手（相对模式）。" },
        { u8"If disabled, use absolute hand pos/angles (MockVRLeft/RightHandPos/Ang).",
          u8"关闭后使用左右手的绝对位置/角度（MockVRLeft/RightHandPos/Ang）。" },
        0.0f, 0.0f,
        "true"
    },
    {
        "MockVRHandPitchOffset",
        OptionType::Float,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Hand Pitch Offset (deg)", u8"手部俯仰偏移（度）" },
        { u8"Additional pitch applied to mock controller orientation.",
          u8"对模拟手柄姿态额外增加的俯仰角偏移。" },
        { u8"Helps match typical VR controller tilt.",
          u8"用于模拟真实手柄常见的倾斜角度。" },
        -180.0f, 180.0f,
        "45"
    },
    {
        ...
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"HMD Position (x,y,z)", u8"HMD位置 (x,y,z)" },
        { u8"Initial / scripted mock HMD position (meters).",
          u8"模拟HMD的初始/脚本位置（米）。" },
        { u8"Used as the base pose for generated tracking data.",
          u8"作为生成追踪数据的基础姿态。" },
        -9999.0f, 9999.0f,
        "0,0,1.65"
    },
    {
        ...
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"HMD Angles (pitch,yaw,roll)", u8"HMD角度 (俯仰,偏航,翻滚)" },
        { u8"Initial / scripted mock HMD angles (degrees).",
          u8"模拟HMD的初始/脚本角度（度）。" },
        { u8"Mouse look adds to these angles when enabled.",
          u8"开启鼠标视角时，会在此基础上叠加。" },
        -180.0f, 180.0f,
        "0,0,0"
    },
    {
        "MockVRLeftHandOffset",
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Left Hand Offset (x,y,z)", u8"左手相对偏移 (x,y,z)" },
        { u8"Left controller offset in HMD local space (meters) when Hands Follow HMD is enabled.",
          u8"开启双手跟随HMD时，左手在HMD本地坐标系下的偏移（米）。" },
        { u8"Typical values: forward, left, down.",
          u8"常见方向：向前、向左、向下。" },
        -5.0f, 5.0f,
        "0.25,0.18,-0.35"
    },
    {
        "MockVRRightHandOffset",
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Right Hand Offset (x,y,z)", u8"右手相对偏移 (x,y,z)" },
        { u8"Right controller offset in HMD local space (meters) when Hands Follow HMD is enabled.",
          u8"开启双手跟随HMD时，右手在HMD本地坐标系下的偏移（米）。" },
        { u8"Typical values: forward, right, down.",
          u8"常见方向：向前、向右、向下。" },
        -5.0f, 5.0f,
        "0.25,-0.18,-0.35"
    },
    {
        ...
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Left Hand Position (x,y,z)", u8"左手绝对位置 (x,y,z)" },
        { u8"Absolute left controller position (meters) when Hands Follow HMD is disabled.",
          u8"关闭双手跟随HMD时，左手的绝对位置（米）。" },
        { u8"Only used in absolute-hand mode.",
          u8"仅在绝对手模式下使用。" },
        -9999.0f, 9999.0f,
        "0.25,0.18,1.30"
    },
    {
        ...
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Right Hand Position (x,y,z)", u8"右手绝对位置 (x,y,z)" },
        { u8"Absolute right controller position (meters) when Hands Follow HMD is disabled.",
          u8"关闭双手跟随HMD时，右手的绝对位置（米）。" },
        { u8"Only used in absolute-hand mode.",
          u8"仅在绝对手模式下使用。" },
        -9999.0f, 9999.0f,
        "0.25,-0.18,1.30"
    },
    {
        ...
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Left Hand Angles (pitch,yaw,roll)", u8"左手角度 (俯仰,偏航,翻滚)" },
        { u8"Absolute left controller angles (degrees) when Hands Follow HMD is disabled.",
          u8"关闭双手跟随HMD时，左手的绝对角度（度）。" },
        { u8"Only used in absolute-hand mode.",
          u8"仅在绝对手模式下使用。" },
        -180.0f, 180.0f,
        "45,0,0"
    },
    {
        ...
        OptionType::Vec3,
        { u8"Mock VR", u8"模拟VR" },
        { u8"Right Hand Angles (pitch,yaw,roll)", u8"右手角度 (俯仰,偏航,翻滚)" },
        { u8"Absolute right controller angles (degrees) when Hands Follow HMD is disabled.",
          u8"关闭双手跟随HMD时，右手的绝对角度（度）。" },
        { u8"Only used in absolute-hand mode.",
          u8"仅在绝对手模式下使用。" },
        -180.0f, 180.0f,
        "45,0,0"
